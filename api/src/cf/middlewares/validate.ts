...
type InferSchema<T extends ZodType> = T['_output'];
...
export function getValidatedJson<TSchema extends ZodType>(c: Context<Env>): InferSchema<TSchema> {
  return c.get('validatedJson') as InferSchema<TSchema>;
}

export function getValidatedFormData<TSchema extends ZodType>(c: Context<Env>): InferSchema<TSchema> {
