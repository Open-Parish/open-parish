...
function buildBody(certificateType: string, record: Record<string, unknown>, title: string): string {
...
export function buildCertificateHtml(input: {
  ...
  const rightImageUrl = resolveAssetUrl(settings.pdfImageRight || settings.pdfImageLeft, baseUrl);
  const signatureUrl = resolveAssetUrl(settings.currentPriestSignature, baseUrl);
